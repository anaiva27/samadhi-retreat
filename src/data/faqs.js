export const faqItems = [
	{
		question: "Is this retreat right for me?",
		answer:
			"If you’re feeling the pull to travel, grow, connect, and rediscover yourself, this might be your sign to say YES! Our retreats are perfect for souls ready to step into a safe, supportive space for transformation. Whether you’re seeking clarity, connection with like-minded souls, or simply a break from your day to day reality, a retreat can be the catalyst for the change you’ve been longing for. Trust your inner knowing - it’s time to honor yourself.",
	},
	{
		question: "Where is the retreat located?",
		answer:
			"The Samadhi retreat center located in the Urubamba Sacred Valley, Peru, nestled between Cusco and Machu Picchu. It’s truly the best venue built with so much love and intention. Here their official website : www.samadhisacredvalley.com",
	},
	{
		question: "What’s not included?",
		answer:
			"International flights to Peru ,Domestic flight to Cusco, Travel insurance (highly recommended), Additional excursions or private sessions (massage, photoshoot and etc) outside the itinerary, Tips for staff",
	},
	{
		question: "What is the daily schedule like?",
		answer:
			"Here is our tentative schedule for our retreat and it’s a subject to change as we are approaching the dates of the retreat : ",
	},
	{
		question: "What should I pack to Perú?",
		answer:
			"Here is our tentative schedule for our retreat and it’s a subject to change as we are approaching the dates of the retreat : ",
	},
	{
		question: "What level of fitness is required?",
		answer:
			"All levels are welcome! Our activities are adaptable to different fitness levels. You’ll be supported whether you’re new to movement or experienced.",
	},
	{
		question: "Is the an age cap?",
		answer:
			"No. All ages, sexual orientation, skin colors, and backgrounds are welcome! Last retreat our participant's age was ranging from 28 to 73. Diverse, different and so freaking beautiful! We believe inter generational and multicultural mix of women make the connection and healing very special. We are all one in the end.",
	},
	{
		question: "I’m coming alone. Will I feel out of place?",
		answer:
			"Not at all! Many guests come solo. This is a supportive space to connect with like-minded people and also enjoy personal reflection. Of course, if you’d like to bring a friend, you’re more than welcome to!",
	},
	{
		question: "Can I meet the group before the retreat?",
		answer:
			"Yes! About a month before the retreat, we’ll create a group chat so everyone can introduce themselves. We also host an optional video call two weeks before the retreat to connect and get excited. After the retreat, there’s an integration call 2-3 weeks later to reflect and stay connected.",
	},
	{
		question: "Do I need travel insurance?",
		answer:
			"Although it's not mandatory, we highly recommend purchasing travel insurance to protect your investment. Because all payments are non-refundable and non-transferable, travel insurance can offer peace of mind in the event of unexpected changes, delays, or emergencies that may affect your ability to attend.",
	},
	{
		question: "Will I be sharing a room?",
		answer: "The chakra bungalows shared between 2 or 3 people.",
	},
	{
		question: "What is the cancellation and refund policy? ",
		answer:
			"A non-refundable deposit of $799 is required to secure your reservation. 💰next two payments $1267 each has to be submitted by April 19th of 2026 and July 19th of 2026. Once payment is submitted it’s not refundable. 💰 If participants choose the 4 days add on after the retreat it is $1199 and it’s non refundable.",
	},
	{
		question: "How do I get to the retreat center? ",
		answer:
			"You’ll fly into Cusco Airport (CUZ). From there, we’ll arrange group transport to the retreat center in the Sacred Valley (approx. 1.5–2 hrs drive).",
	},
	{
		question:
			"I need help with figuring out my flights, would you help me with that?",
		answer: "Yes of course, let’s us help you to find the best flight for you.",
	},
	{
		question: "What if I have dietary restrictions?",
		answer:
			"Our chef can accommodate most dietary needs including vegetarian, vegan, gluten-free, and dairy-free. Please let us know when booking. ",
	},
	{
		question: "Is Peru safe for travelers?",
		answer:
			"Yes, especially in the Sacred Valley. We work with trusted local partners and prioritize safety throughout the retreat. ",
	},
	{
		question:
			"I’m interested but want to learn more - can I talk to someone first?",
		answer:
			"Absolutely! We offer short video calls with our team for anyone interested in joining a retreat. It’s a chance for you to meet us face-to-face, feel the vibe, ask questions, and learn more about the experience to make sure it’s a true full-body yes for you. We’re happy to support and make sure it’s the perfect fit",
	},
];
